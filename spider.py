...
def get_news_list(category):
    ...
    if category == '高层动态':
        ...
        while True:
            ...
            try:
                re = requests.get(url='https://www.yidaiyilu.gov.cn/info/iList.jsp?cat_id=10149&cur_page={}'.format(str(pages)),
                               headers=header)
            except:
                print('服务器已限制访问，请等待30s后自动重试')
                time.sleep(30)
                re = requests.get(url='https://www.yidaiyilu.gov.cn/info/iList.jsp?cat_id=10149&cur_page={}'.format(str(pages)), headers=header)
            re.encoding = 'UTF-8'
            html = re.text
        ...
        while True:
            print('正在读取第{}页'.format(str(pages)))
            try:
                re = requests.get(url='https://www.yidaiyilu.gov.cn/info/iList.jsp?cat_id=10005&cur_page={}'.format(str(pages)), headers=header)
            except:
                print('服务器已限制访问，请等待30s后自动重试')
                time.sleep(30)
                re = requests.get(url='https://www.yidaiyilu.gov.cn/info/iList.jsp?cat_id=10005&cur_page={}'.format(str(pages)),
                    headers=header)
            re.encoding = 'UTF-8'
            html = re.text
            soup = BeautifulSoup(html, features="html.parser")
